...
| `src/fa-solid-900-subset.woff2` | Font Awesome 7.3.1 Free **Solid**, subset to the 15 glyphs the panel uses (119 KB → 2.1 KB). Regenerate with `.workbuddy/fa_subset.py` after editing its `ICONS` list, then add the printed codepoint as a `.fa-<name>` rule in `newtab.css`. |
...
| `searchRadius` | `8` | Corner radius in px, `modern` only. `0` means "keep the default". Allowed 0–60 |
| `searchBorder` | `1` | Border weight in px, both styles. `0` is refused — allowed 1–10 |
| `searchLength` | `500` | Search box width in px, both styles. `0` means "keep the default". Allowed 200–1200 |
...
- `SEARCH_LIMITS` in `newtab.js` is the single source of truth for the allowed ranges and the defaults; `readLimit()` resolves anything missing, non-integer, out of range, or a "keep the default" 0 to `spec.default`, and `isValidLimit()` is what the dialog validates against — the two share the rule so a rejected input and a stored value can never disagree about what 0 means.
...
- Border weight has `min: 1` / `zeroMeansDefault: false`: a frameless box is what `geek` already is, so 0 is refused with an inline error rather than silently reinterpreted.
- The range hint shown in the dialog's placeholder comes from `rangeWithDefault` / `rangeNoZero` in `I18N`, selected by `spec.zeroMeansDefault` — never hard-code a range in the markup, or the field will advertise limits the validator does not enforce.
