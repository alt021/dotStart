...
- `actionTitle` lives in **both** `_locales/*/messages.json` (for `__MSG_actionTitle__`, which resolves against the browser's UI locale) **and** `I18N` in both languages. The page calls `chrome.action.setTitle()` at `DOMContentLoaded` so the tooltip follows the language chosen *in the app*; the manifest value remains the fallback for the window before a new tab has ever loaded. The dialog notice quotes that same string — it is the only thing pointing a panicking user at the button.
