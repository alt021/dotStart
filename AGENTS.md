...
| `src/fa-solid-900-subset.woff2` | Font Awesome 7.3.1 Free **Solid**, subset to the 12 glyphs the panel uses (119 KB → 2 KB). Regenerate with `.workbuddy/fa_subset.py` after editing its `ICONS` list, then add the printed codepoint as a `.fa-<name>` rule in `newtab.css`. |
...
- A tile is **square**, not fixed-width: `flex: 0 0 auto; height: 100%; aspect-ratio: 1 / 1`. The strip height (fixed panel height minus tabs and padding) defines the side, so tiles can never stretch into rectangles when the window resizes. Inside a tile the icon plate takes the free space, then a hairline, then name + current value, all centred.
